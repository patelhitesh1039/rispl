...
{{-- @if ($totalRecords > 0 && Auth::user()->role_id == 1)
...
<a class="btn btn-primary shadow-md mr-2 mt-10 d-inline addbtn downloadExcel">Excel</a>
        ...
        <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="w-56 relative text-slate-500" style="display:inline-block">
                    ...
                    @else
                    <a href=""><i class="w-4 h-4 absolute my-auto inset-y-0 mr-3 right-0"
                            data-lucide="x"></i></a>
                    @endif
                </div>
    ...
    <table class="table table-report mt-2" aria-label="customer-list">
        <thead class="sticky-top">
            <tr>
                ...
                <th class="text-center whitespace-nowrap">BRANCH</th>

                <th class="text-center whitespace-nowrap">GATE PASS</th>
                <th class="text-center whitespace-nowrap">AADHARCARD PASS</th>
        ...
        <tbody id="todo-list">
            ...
            <tr class="intro-x">
                ...
                <td class="text-center">
                    {{ date('d-m-Y', strtotime($user->joiningDate)) ? date('d-m-Y', strtotime($user->joiningDate)) :
                    '--'
                    }}
                </td>
                <td class="text-center">{{ $user->getBranch ? $user->getBranch->name : '--' }}</td>

                <td class="text-center">{{ $user->gatepass ? $user->gatepass : '--' }}</td>
                <td class="text-center">{{ $user->aadharcard ? $user->aadharcard : '--' }}</td>
...
<div class="d-inline intro-y col-span-12 addbtn ">
    <nav class="w-full sm:w-auto sm:mr-auto">
        <ul class="pagination" id="pagination">
            ...
            @for ($i = 0; $i < $totalPages; $i++) <li class="page-item {{ $page == $i + 1 ? 'active' : '' }} ">
                <a class="page-link"
                    href="">{{ $i + 1
                    }}</a>
                </li>
...
{{-- <script type="text/javascript">
    var spinner = $('.loader');
        jQuery(function() {
            ...
            jQuery('.downloadExcel').click(function(e) {
                e.preventDefault();
                spinner.show();
                var searchString = $("#searchString").val();
                jQuery.ajax({
